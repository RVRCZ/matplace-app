...
final class ModelNormalizer
{
    ...
    public function toPrintableStl(string $inPath, string $outPath, float $targetMaxMm, bool $yUp = true): string
    {
        $targetMaxMm = max(5.0, min(1000.0, $targetMaxMm));
        $ext = strtolower(pathinfo($inPath, PATHINFO_EXTENSION));

        if ($this->python->available()) {
            $r = $this->python->run(['normalize', $inPath, $outPath, (string) $targetMaxMm, $yUp ? '1' : '0']);
