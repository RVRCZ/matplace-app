...
export function setDownload(file: FileInfo | null, params: () => DownloadParams, dims: () => { x: number; y: number; z: number } | null): void {
    ...
    if (parts) {
        ...
        parts.innerHTML = (file.parts ?? []).length ? `${i18n['download.parts'] ?? ''}: ` + (file.parts ?? []).map((p) => `<a class="text-action-dark underline" href="${routes().paramPart}/${file.uuid}/${p}.stl">${p.startsWith('bin_') ? (i18n['param.part.bin'] ?? ':s').replace(':s', p.slice(4).replace('x', ' × ')) : (i18n[`param.part.${p}`] ?? p)}</a>`).join(' · ') : '';
    }
    refresh();
}
