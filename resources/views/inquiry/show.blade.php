            ...
            <div class="overflow-hidden rounded-2xl border border-slate-200 bg-white">
                ...
                <div class="p-4 text-sm">
                    ...
                    <div class="mt-3 text-xs text-slate-500">{{ __('inquiry.sent_to', ['n' => $inquiry->dispatches->count()]) }}
                        @if($inquiry->calculation)· <a href="{{ route('calc.share', $inquiry->calculation) }}" class="text-teal-700">{{ __('inquiry.open_calc') }}</a>@endif
                    </div>
                </div>
            </div>
        ...
        <div class="space-y-3">
                ...
                <div class="rounded-2xl border {{ $isAccepted ? 'border-teal-500' : 'border-slate-200' }} bg-white p-4">
                    ...
                    <div class="mt-3 flex flex-wrap gap-2">
                        @if($inquiry->isOpenForOffers() && in_array($o->status, ['sent', 'viewed']))
                            <form method="post" action="{{ route('inquiry.accept', [$inquiry, $o]) }}">@csrf<button class="rounded-xl bg-teal-600 px-4 py-2 font-semibold text-white">{{ __('inquiry.accept') }}</button></form>
                        @elseif($isAccepted)
                            <span class="rounded-xl bg-teal-50 px-4 py-2 font-semibold text-teal-800">✅ {{ __('inquiry.you_accepted') }}</span>
                        @endif
            ...
            @if($inquiry->status === 'accepted')
                <form method="post" action="{{ route('inquiry.done', $inquiry) }}" class="rounded-2xl border border-slate-200 bg-white p-4">@csrf
                    ...
                    <button class="mt-2 rounded-xl border border-teal-600 px-4 py-2 font-semibold text-teal-700">{{ __('inquiry.mark_done') }}</button>
                </form>
            @endif
            @if($inquiry->status === 'done' && !$rated)
                <form method="post" action="{{ route('inquiry.rate', $inquiry) }}" class="rounded-2xl border border-slate-200 bg-white p-4">@csrf
                    ...
                    <button class="mt-2 rounded-xl bg-teal-600 px-4 py-2 font-semibold text-white">{{ __('inquiry.rate_submit') }}</button>
                </form>
            @elseif($rated)
