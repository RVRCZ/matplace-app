...
<div class="mt-3 space-y-3">
    @foreach($printers as $p)
        ...
        <a href="{{ route('admin.farm.printers.edit', $p) }}" class="block rounded-2xl border border-slate-200 bg-white p-4 hover:border-action">
            ...
            <p class="mt-1 text-sm text-slate-600">{{ (int) $p->bed_x }} × {{ (int) $p->bed_y }} × {{ (int) $p->bed_z }} mm · {{ $p->nozzle_mm }} mm · čas × {{ $p->time_factor }} · hmotnost × {{ $p->weight_factor }}@if($p->hourly_rate) · {{ $p->hourly_rate }} Kč/h @endif</p>
