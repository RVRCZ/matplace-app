...
<div class="mx-auto max-w-2xl">
    ...
    <form id="figure-form" class="mt-5 space-y-4 rounded-2xl border border-slate-200 bg-white p-5">
        <div class="grid grid-cols-2 gap-2">
            <label class="cursor-pointer rounded-xl border border-slate-300 p-3 text-center has-[:checked]:border-teal-600 has-[:checked]:bg-teal-50">
        ...
        <label class="flex cursor-pointer flex-col items-center justify-center rounded-2xl border-2 border-dashed border-teal-300 px-4 py-8 text-center hover:border-teal-500 hover:bg-teal-50">
        ...
        <label class="block text-sm font-semibold">{{ __('figure.size') }} <span id="figure-size-val" class="font-normal text-teal-700">80 mm</span>
            <input id="figure-size" name="target_mm" type="range" min="30" max="250" step="5" value="80" class="mt-1 w-full accent-teal-600">
        </label>
        ...
        <button class="w-full rounded-xl bg-teal-600 px-4 py-3 font-semibold text-white disabled:opacity-60">{{ __('figure.submit') }}</button>
        <div id="figure-progress" class="hidden"><div class="h-2 w-full overflow-hidden rounded-full bg-slate-200"><div id="figure-bar" class="h-2 w-0 bg-teal-600 transition-all"></div></div></div>
